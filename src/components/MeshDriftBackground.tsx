...
/**
 * MeshDriftBackground
 * Plain WebGL1 Fullscreen Triangle Shader Background — "Mesh drift" (blobs shader)
 * Generated with 21st.dev Shader Builder specifications.
 * 
 * Colours (low -> high): #101010, #F5F5F5, #B0B0B0, #3A3A3A
 * Feel: speed 33/100, zoom 33/100, intensity 34/100, warp 0/100, contrast 62/100,
 *       brightness 50/100, saturation 50/100, hue 0°, vignette 0/100, grain 26/100
 * Cursor: off
 */
...
export const MeshDriftBackground: React.FC<MeshDriftBackgroundProps> = ({ className = '' }) => {
  ...
  useEffect(() => {
    ...
    const gl =
      ...
      ((canvas.getContext('experimental-webgl') as WebGLRenderingContext | null) ?? null);

    if (!gl) {
      console.warn('WebGL1 context unavailable for Mesh Drift background.');
      return;
    }
    ...
    // Canonical fullscreen triangle covering clip-space [-1, 1] x [-1, 1]
    ...
    // Uniform locations
    const uColorsLocation =
      gl.getUniformLocation(program, 'u_colors[0]') || gl.getUniformLocation(program, 'u_colors');
    ...
    // Packed 8-color array (first 4 used for #101010, #F5F5F5, #B0B0B0, #3A3A3A)
    ...
    const render = (now: number) => {
      ...
      // Uniform 1: u_colors[8]
      if (uColorsLocation) {
        gl.uniform3fv(uColorsLocation, colorsData);
      }

      // Uniform 2: u_scene = vec4(canvas width, canvas height, seconds * 0.73, 4.0)
      if (uSceneLocation) {
        gl.uniform4f(uSceneLocation, canvas.width, canvas.height, accumulatedSeconds * 0.73, 4.0);
      }

      // Uniform 3: u_shape = vec4(1.16, 0.34, 0.50, 0.00)
      if (uShapeLocation) {
        gl.uniform4f(uShapeLocation, 1.16, 0.34, 0.50, 0.00);
      }

      // Uniform 4: u_surface = vec4(2.40, 1.16, 0.00, 1.00)
      if (uSurfaceLocation) {
        gl.uniform4f(uSurfaceLocation, 2.40, 1.16, 0.00, 1.00);
      }

      // Uniform 5: u_finish = vec4(0.00, 0.00, 0.000, 0.09)
      if (uFinishLocation) {
        gl.uniform4f(uFinishLocation, 0.00, 0.00, 0.000, 0.09);
      }

      // Uniform 6: u_transform = vec4(1453.0, 0.00, 0.00, 0.0)
      if (uTransformLocation) {
        gl.uniform4f(uTransformLocation, 1453.0, 0.00, 0.00, 0.0);
      }

      // Uniform 7: u_space = vec4(0.00, 0.00, pointer x, pointer y)
      if (uSpaceLocation) {
        gl.uniform4f(uSpaceLocation, 0.00, 0.00, 0.00, 0.00);
      }

      // Uniform 8: u_cursor = vec4(presence, 2.0, 0.65, 0.46) — Cursor: off
      if (uCursorLocation) {
        gl.uniform4f(uCursorLocation, 0.0, 2.0, 0.65, 0.46);
      }
  ...
  return (
    ...
    >
      <canvas
        ref={canvasRef}
        id="mesh-drift-canvas"
        className="mesh-drift-canvas"
        aria-hidden="true"
        style={{
          position: 'absolute',
          top: 0,
