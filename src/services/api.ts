...
export class GalleryService {
  ...
  static canUserManageArtwork(artwork: Artwork | null | undefined, userOrId: UserProfile | string | null | undefined): boolean {
    ...
    // STRICT GUEST PERMISSION GATE:
    // Guests cannot edit or delete ANY artwork (neither other artists' work nor their own works uploaded as a guest).
    // Edit and delete permissions are enabled ONLY for users who sign in or create an artist profile.
    if (!userId || userId === 'guest' || userHandle === '@visitor') {
      return false;
    }
    ...
    // For registered / signed-in users: allow management if this browser session authored the artwork
    if (isClientAuthor(artwork.id)) return true;

    return (
      (artwork.artist.id && artwork.artist.id !== 'guest' && !artwork.artist.id.startsWith('guest-') && artwork.artist.id.toLowerCase() === userId.toLowerCase()) ||
      (artwork.artist.handle && artwork.artist.handle !== '@visitor' && !artwork.artist.handle.startsWith('@guest_') && !artwork.artist.handle.startsWith('@scribe_') && userHandle && artwork.artist.handle.toLowerCase() === userHandle.toLowerCase())
    );
  }
  ...
  static deleteArtwork(id: string, requester?: UserProfile | string): { success: boolean; message: string } {
    ...
    const requesterId = typeof currentRequester === 'string' ? currentRequester : currentRequester.id;
    const requesterHandle = typeof currentRequester === 'string' ? currentRequester : currentRequester.handle;

    // Strict Guest Protection: Guests cannot delete ANY artwork
    if (!requesterId || requesterId === 'guest' || requesterHandle === '@visitor') {
      return {
        success: false,
        message: 'Unauthorized: Guests cannot delete artworks. Please sign in or create an artist profile to manage creations.'
      };
    }
  ...
  static restoreArtwork(id: string, requester?: UserProfile | string): { success: boolean; message: string } {
    ...
    // Strict Guest Protection: Guests cannot restore artworks
    if (!requesterId || requesterId === 'guest' || requesterHandle === '@visitor') {
      return {
        success: false,
        message: 'Unauthorized: Guests cannot restore artworks. Please sign in or create an artist profile.'
      };
    }
  ...
  static permanentlyDeleteArtwork(id: string, requester?: UserProfile | string): { success: boolean; message: string } {
    ...
    // Strict Guest Protection: Guests cannot purge artworks
    if (!requesterId || requesterId === 'guest' || requesterHandle === '@visitor') {
      return {
        success: false,
        message: 'Unauthorized: Guests cannot purge artworks. Please sign in or create an artist profile.'
      };
    }
  ...
  static updateArtwork(
    ...
    // Strict Guest Protection: Guests cannot edit ANY artwork
    if (!requesterId || requesterId === 'guest' || requesterHandle === '@visitor') {
      return {
        error: 'Unauthorized: Guests cannot edit artworks. Please sign in or create an artist profile to manage creations.'
      };
    }
  ...
  static createArtwork(artworkData: Partial<Artwork>): Artwork {
    ...
    // Record client-side authorship for authenticated artists with profiles
    if (!isGuest) {
